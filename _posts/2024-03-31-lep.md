...
## Pourquoi un Pattern ?
Comme l'explique Martin Folwer [sur son site](https://www.martinfowler.com/bliki/POJO.html), les patterns sont plus populaires lorsqu'on leur donne un nom sympa. En donnant un nom distinctif au LEP, nous pouvons mieux communiquer sur cette pratique et ainsi encourager son adoption.
...
Le LEP propose une solution simple à ce problème. L'idée est d'encapsuler la liste d'articles dans une classe dédiée, ce qui nous permet de centraliser la logique de traitement associée à cette liste. Voici comment nous pourrions réécrire notre exemple en utilisant le LEP :
```java
public record Item(double price) {
}
